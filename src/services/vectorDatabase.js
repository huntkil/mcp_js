    ...
    try {
      ...
      function cosine(a, b) {
        ...
        for (let i = 0; i < a.length; i++) {
          if (isNaN(a[i]) || !isFinite(a[i]) || isNaN(b[i]) || !isFinite(b[i])) return 0;
        }
      ...
      for (const v of this.vectors.values()) {
        try {
          ...
          if (!vector || !Array.isArray(vector)) {
            badVectorWarnCount++;
            if (badVectorWarnCount <= 10) logger.warn(`벡터 데이터 없음 또는 잘못된 형태: ${v.id}`);
            continue;
          }
          const score = cosine(queryVector, vector);
          if (isNaN(score)) {
            nanCount++;
            if (nanWarnCount <= 10) logger.warn(`유사도 점수가 NaN: ${v.id}`);
            nanWarnCount++;
            continue;
          }
          // 상위 점수 추적
      ...
      // 디버깅 로그(최소화)
      logger.info(`[VECTOR][DEBUG] 검색 통계: 처리 ${processedCount}개, 필터링 ${filteredCount}개, NaN ${nanCount}개, 벡터오류 ${badVectorWarnCount}개`);
